...
import { addModalMovie, clearModalMovie, addBookingModalShow, clearBookingModal, showConfirmModal, clearConfirmModal, toggleThankYouModal } from '../redux/actions'
import { dispatch } from '../redux/store'

class Modals extends Component {

  toggleMovieModal = (movie) => {
    movie && movie.Title ? dispatch(addModalMovie(movie)) : dispatch(clearModalMovie());
  };

  toggleBookingModal = (show) => {
    show && !show.target ? dispatch(addBookingModalShow(show)) : dispatch(clearBookingModal());
  };

  toggleConfirmModal = (show) => {

    show && !show.target ? dispatch(showConfirmModal(show)) : dispatch(clearConfirmModal());
  };
