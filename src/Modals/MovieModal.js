...
import { addBookingModalShow, getSchedule } from '../redux/actions'
import { dispatch } from '../redux/store'

const processAreas = (arr, movieId) => arr.reduce((acc, cur) => {
  return [...acc, ...processDates(Object.values(cur), movieId)]
}, [])
const processDates = (arr, movieId) => arr.reduce((acc, cur) => {
  return [...acc, ...processSchedule(Object.values(cur), movieId)]
}, [] );
...
class MovieModal extends React.Component {
  ...
  componentWillReceiveProps = (nextProps) => {
    if (nextProps.movie !== null && nextProps.movie !== this.props.movie) dispatch(getSchedule({eventID: nextProps.movie.ID, nrOfDays: 31, area: nextProps.area.ID}))
  }
